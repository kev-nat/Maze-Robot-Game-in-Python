...
model.load_state_dict(torch.load(r'D:\Maze Robot Game\SavedNets\nn_bot.pkl'))
BotStartLocation =  2
...
class BotEnv:    
    ...
    def BuildBot(self, x, y, r):
        ...
        BoxPoints = list(map(lambda point: Vec2d(point[0], point[1]), [(-size, -size), (-size, size), (size, size), (size, -size)]))
        mass  = 0.5
        ...
        angle_coordinates = PointsFromAngle(self.Bot.angle)
        
        BotDirection = Vec2d(angle_coordinates[0], angle_coordinates[1])
    ...
    def DrawEverything(self,flag=0):
        ...
        img = pygame.image.load(r"D:\Maze Robot Game\assets\intel.jpg")
        x, y = 580,550
        ...
        img = pygame.image.load(r"D:\Maze Robot Game\assets\spherelight.png")
        
        # offset = Vec2d(img.get_size()) / 2.
        offset = Vec2d(img.get_size()[0] / 2., img.get_size()[1] / 2.)
    ...
    def _step(self, action, CrashStep=0):
        ...
        if action == 3:  
            ...
            angle_coordinates = PointsFromAngle(self.Bot.angle)
            self.BotDirection = Vec2d(*angle_coordinates)
        ...
        # If Action Is Right
            ...
            # self.BotDirection = Vec2d(PointsFromAngle(self.Bot.angle))
            angle_coordinates = PointsFromAngle(self.Bot.angle)
            self.BotDirection = Vec2d(x=angle_coordinates[0], y=angle_coordinates[1])
            BotDirection = self.BotDirection
        ...
        # If Action Is Straight
            ...
            if(between_angle > 0.15):
                ...
                if(d > 0):
                        ...
                        # self.BotDirection = Vec2d(PointsFromAngle(self.Bot.angle))
                        angle_coordinates = PointsFromAngle(self.Bot.angle)
                        self.BotDirection = Vec2d(angle_coordinates[0], angle_coordinates[1])
                        BotDirection = self.BotDirection
                        self.Bot.velocity = BotSpeed* BotDirection
                ...
                # self.BotDirection = Vec2d(PointsFromAngle(self.Bot.angle))
                angle_coordinates = PointsFromAngle(self.Bot.angle)
                BotDirection = Vec2d(math.cos(angle_coordinates[0]), math.sin(angle_coordinates[1]))
                
                self.BotDirection = BotDirection.normalized()
                BotDirection = self.BotDirection
                self.Bot.velocity = BotSpeed * BotDirection
